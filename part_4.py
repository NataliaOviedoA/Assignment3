...
print(w2i[ '.start']) #index is 1
print(w2i[ '.end']) #index is 2
print(x_train[0])
...
#padded = [batch + [[0] * (max_cols)] * (max_rows - len(batch)) for batch in matrix]
...
for i in range(epochs):
    #Still need to shuffle the list of batches
    # shuffled = matrix.shuffle(buffer_size=5)
    for batch_num in range(len(matrix)):
        ...
        target = [row[:] for row in matrix[batch_num]]
        for row in target:
            del row[1]
            row.append(0)
